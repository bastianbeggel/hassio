...
DOMAIN = 'keyboard_remote2'

ICON = 'mdi:remote'

KEY_CODE = 'key_code'
KEY_VALUE = {'key_up': 0, 'key_down': 1, 'key_hold': 2}
KEYBOARD_REMOTE_COMMAND_RECEIVED = 'keyboard_remote2_command_received'
KEYBOARD_REMOTE_CONNECTED = 'keyboard_remote2_connected'
KEYBOARD_REMOTE_DISCONNECTED = 'keyboard_remote2_disconnected'
...
class KeyboardRemoteThread(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stopped.isSet():
            ...
            if event.value == 1:
                self.state[event.code] = event.timestamp()
            # When it's released, remove it from the state map.
            if event.value == 0 and event.code in self.state:
                    now = time.time()                
